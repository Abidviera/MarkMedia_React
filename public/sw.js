...
self.addEventListener('fetch', (event) => {
  ...
  // Skip video files - streaming content doesn't cache well with Cache API
  if (request.destination === 'video' || url.pathname.endsWith('.mp4')) {
    return;
  }

  // Handle range requests (206) - skip caching for partial responses
  ...
  event.respondWith(
    caches.match(request).then((cachedResponse) => {
      ...
      return fetch(request).then((networkResponse) => {
        // Don't cache if not a full response (206 partial)
